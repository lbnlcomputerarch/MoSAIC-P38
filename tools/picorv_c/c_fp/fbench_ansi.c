...
static double od_sa_ref[2][2] = { 47.09479120920,  0.04178472683,
                                  47.08372160249,  0.04177864821 
                                 };

const static double aberr_lspher_ref = -0.01106960671;
const static double max_lspher_ref   = 0.05306749907;
const static double aberr_osc_ref    = 0.00008954761;
const static double max_osc_ref      = 0.00250000000;
const static double aberr_lchrom_ref = 0.00448229032;
const static double max_lchrom_ref   = 0.05306749907;
...
uint32_t end_exec[600] __attribute__ ((section(".myspad")));  
...
static void transit_surface(void) {
        ...
        if (paraxial) {
           if (radius_of_curvature != 0.0) {
              ...
              #ifdef DEBUG
              uint32_t temp1 = 0xc0c01000 | paraxial;
              w2_mPut(temp1, &end_exec[offset+tile_id_offset]);
              offset++;
              uint32_t temp2 = (uint32_t) object_distance;
              w2_mPut(temp2, &end_exec[offset+tile_id_offset]);
              offset++;
              #endif
              return;
           }
           ...
           #ifdef DEBUG
           uint32_t temp1 = 0xc0c02000 | paraxial;
           w2_mPut(temp1, &end_exec[offset+tile_id_offset]);
           offset++;
           uint32_t temp2 = (uint32_t) object_distance;
           w2_mPut(temp2, &end_exec[offset+tile_id_offset]);
...
static void trace_line(int line, double ray_h)
{
        ...
        for (i = 1; i <= current_surfaces; i++) {
           #ifdef DEBUG
           uint32_t temp1 = 0xf0ca0000|i;
           w2_mPut(temp1, &end_exec[offset+tile_id_offset]);
           offset++;
           uint32_t temp2 = 0xf0ca1000 | (uint32_t)current_surfaces;
           w2_mPut(temp2, &end_exec[offset+tile_id_offset]);
           offset++;
           #endif
...
int main(int argc, char *argv[])
{
   ...
   */
   ...
   temp = 0xcafe0000 | tile_id;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]);
   offset++;

   ball = 5; //- Remove when using qWait
   temp = 0xbaca0000 | ball;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]);
   offset++;
   ...
   for (itercount = 0; itercount < niter; itercount++) {
      ...
      #ifdef DEBUG
      uint32_t temp1 = 0xc0c0a000 | itercount;
      w2_mPut(temp1, &end_exec[offset+tile_id_offset]);
      offset++;
      #endif
      ...
      for (paraxial = 0; paraxial <= 1; paraxial++) {
         ...
         #ifdef DEBUG
         uint32_t temp1 = 0xc0c0a100 | paraxial;
         w2_mPut(temp1, &end_exec[offset+tile_id_offset]);
         offset++;
         #endif
      ...
      #ifdef DEBUG
      uint32_t temp2 = 0xc0c0a200 | paraxial;
      w2_mPut(temp2, &end_exec[offset+tile_id_offset]);
      offset++;
      #endif
      ...
      #ifdef DEBUG
      uint32_t temp3 = 0xc0c0a300 | paraxial;
      w2_mPut(temp3, &end_exec[offset+tile_id_offset]);
      offset++;
      #endif
      ...
      /* D light */
      #ifdef DEBUG
      uint32_t temp4 = 0xc0c0a400 | paraxial;
      w2_mPut(temp4, &end_exec[offset+tile_id_offset]);
      offset++;
      #endif
   ...
   #ifdef DEBUG
   errors_temp = compare(od_sa_ref[0][0], od_sa[0][0]); //- Marginal ray
   #else
   errors_temp = compare1(od_sa_ref[0][0], od_sa[0][0],&end_exec[offset+tile_id_offset]); //- Marginal ray
   offset = offset + 4;
   #endif
   errors = errors_temp;
   temp = 0xfeed0000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;
   
   #ifdef DEBUG
   errors_temp = compare(od_sa_ref[0][1], od_sa[0][1]);
   #else
   errors_temp = compare1(od_sa_ref[0][1], od_sa[0][1], &end_exec[offset+tile_id_offset]);
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed1000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;
   
   #ifdef DEBUG
   errors_temp = compare(od_sa_ref[1][0], od_sa[1][0]); //- Paraxial ray
   #else
   errors_temp = compare1(od_sa_ref[1][0], od_sa[1][0], &end_exec[offset+tile_id_offset]); //- Paraxial ray
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed2000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   #ifdef DEBUG
   errors_temp = compare(od_sa_ref[1][1], od_sa[1][1]);
   #else
   errors_temp = compare1(od_sa_ref[1][1], od_sa[1][1], &end_exec[offset+tile_id_offset]);
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed3000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   #ifdef DEBUG
   errors_temp = compare(aberr_lspher_ref, aberr_lspher);
   #else
   errors_temp = compare1(aberr_lspher_ref, aberr_lspher, &end_exec[offset+tile_id_offset]);
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed4000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   #ifdef DEBUG
   errors_temp = compare(max_lspher_ref, max_lspher);
   #else
   errors_temp = compare1(max_lspher_ref, max_lspher, &end_exec[offset+tile_id_offset]);
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed5000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   #ifdef DEBUG
   errors_temp = compare(aberr_osc_ref, aberr_osc);
   #else
   errors_temp = compare1(aberr_osc_ref, aberr_osc, &end_exec[offset+tile_id_offset]);
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed6000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   #ifdef DEBUG
   errors_temp = compare(max_osc_ref, max_osc);
   #else
   errors_temp = compare1(max_osc_ref, max_osc, &end_exec[offset+tile_id_offset]);
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed7000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   #ifdef DEBUG
   errors_temp = compare(aberr_lchrom_ref, aberr_lchrom);
   #else
   errors_temp = compare1(aberr_lchrom_ref, aberr_lchrom, &end_exec[offset+tile_id_offset]);
   offset = offset + 4;
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed8000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   #ifdef DEBUG
   errors_temp = compare(max_lchrom_ref, max_lchrom);
   #else
   errors_temp = compare1(max_lchrom_ref, max_lchrom, &end_exec[offset+tile_id_offset]);
   offset = offset+ 4; 
   #endif
   errors = errors + errors_temp;
   temp = 0xfeed9000 | errors_temp;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++; 

   /* Marginal ray od_sa[0][0] od_sa[0][1]
    * Paraxial ray od_sa[1][0] od_sa[1][1]
    * Longitudinal spherical aberration     : aberr_lspher
    *    Maximum Permissible                : max_lspher
    * Offsense against sine condition (coma): aberr_osc
    *    Maximum Permissible                : max_osc
    * Axial Chromatic aberration            : aberr_lchrom
    *    Maximum Permissible                : max_lchrom   
    *
    * Marginal ray          47.09479120920    0.04178472683
    * Paraxial ray          47.08372160249    0.04177864821
    * Longitudinal spherical aberration    : -0.01106960671
   ...
   temp = 0xc0ca0000 | errors;
   w2_mPut(temp, &end_exec[offset+tile_id_offset]); 
   offset++;

   ball = ball + 1;
   qPut(dest_tile, ball);  // qPut(dest, data)
